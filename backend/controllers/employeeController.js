...
const createEmployee = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [id_number, fullname, hire_date, OWNER_ID]
    );
...
const updateEmployee = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE employees
       SET id_number=$1, fullname=$2, hire_date=$3, termination_date=$4, active=$5
       WHERE employeeid=$6 AND userid=$7 RETURNING *`,
      [id_number, fullname, hire_date, termination_date || null, active, id, OWNER_ID]
    );
    if (result.rows.length === 0)
      return res.status(404).json({ error: 'Empleado no encontrado' });
    res.json(result.rows[0]);
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE employees SET active = false
       WHERE employeeid = $1 AND userid = $2 RETURNING *`,
      [id, OWNER_ID]
    );
    if (result.rows.length === 0)
      return res.status(404).json({ error: 'Empleado no encontrado' });
